...
{
				echo "isset";
				$stmt = $mysqli->prepare("UPDATE fan SET fid=?,f_fname=?,f_mname=?,f_lname=?,f_city=?,trustscore=?,lastlogintime=?,lastlogouttime=?,f_birthdate=?,f_email=?,f_street=?,f_zipcode=?,password=? WHERE fid='{$fid}'");
}
				$fid=$_GET['id'];
				...
				$stmt->bind_param('sssssssisssis',$fid,$f_fname,$f_mname,$f_lname,$f_city,$trustscore,$lastlogintime,$lastlogouttime,$f_birthdate,$f_email,$f_street,$f_zipcode,$password);
